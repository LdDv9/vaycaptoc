	...
	<form name="lostpasswordform" id="lostpasswordform<?php $template->the_instance(); ?>" action="<?php $template->the_action_url( 'lostpassword', 'login_post' ); ?>" method="post">
		<p class="tml-user-login-wrap">
			...
			if ( 'email' == $theme_my_login->get_option( 'login_type' ) ) {
				...
				_e( 'Tên tài khoản hoặc E-mail:', 'theme-my-login' );
			} ?></label>
			<input placeholder="Tên tài khoản hoặc E-mail" type="text" name="user_login" id="user_login<?php $template->the_instance(); ?>" class="input" value="<?php $template->the_posted_value( 'user_login' ); ?>" size="20" />
		</p>

		<?php do_action( 'lostpassword_form' ); ?>

		<p class="tml-submit-wrap">
			<input type="submit" name="wp-submit" id="wp-submit<?php $template->the_instance(); ?>" value="<?php esc_attr_e( 'Lấy lại mật khẩu', 'theme-my-login' ); ?>" />
