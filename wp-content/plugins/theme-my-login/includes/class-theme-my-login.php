			...
			switch ( $this->request_action ) {
				...
				default:
					...
					if		( isset( $_GET['loggedout'] ) && true == $_GET['loggedout'] )
						$this->errors->add( 'loggedout', __( 'Bạn đã đăng xuất.', 'theme-my-login' ), 'message' );
					elseif	( isset( $_GET['registration'] ) && 'disabled' == $_GET['registration'] )
						$this->errors->add( 'registerdisabled', __( 'Đăng ký người dùng hiện không được cho phép.', 'theme-my-login' ) );
					elseif	( isset( $_GET['checkemail'] ) && 'confirm' == $_GET['checkemail'] )
						$this->errors->add( 'confirm', __( 'Vui lòng kiểm tra Email để xác nhận link', 'theme-my-login' ), 'message' );
					elseif ( isset( $_GET['resetpass'] ) && 'complete' == $_GET['resetpass'] )
						$this->errors->add( 'password_reset', __( 'Mật khẩu của bạn đã được tạo mới.', 'theme-my-login' ), 'message' );
					elseif	( isset( $_GET['checkemail'] ) && 'registered' == $_GET['checkemail'] )
						$this->errors->add( 'registered', __( 'Đăng ký thành công, vui lòng kiểm tra E-mail.', 'theme-my-login' ), 'message' );
					elseif	( $interim_login )
						$this->errors->add( 'expired', __( 'Phiên của bạn đã hết hạn, vui lòng đăng nhập lại.', 'theme-my-login' ), 'message' );
					elseif ( strpos( $redirect_to, 'about.php?updated' ) )
						$this->errors->add('updated', __( '<strong>Bạn đã cập nhật thành công WordPress!</strong> Vui lòng đăng nhập lại để trải nghiệm sự khiếp sợ.', 'theme-my-login' ), 'message' );
					elseif	( $reauth )
						$this->errors->add( 'reauth', __( 'Vui lòng đăng nhập để tiếp tục.', 'theme-my-login' ), 'message' );
...
<?php
	...
	public static function retrieve_password() {
		...
		if ( empty( $_POST['user_login'] ) ) {
			$errors->add( 'empty_username', __( '<strong>Lỗi</strong>: Nhập tên tài khoản hoặc địa chỉ mail.', 'theme-my-login' ) );
		} else if ( strpos( $_POST['user_login'], '@' ) ) {
			$user_data = get_user_by( 'email', trim( wp_unslash( $_POST['user_login'] ) ) );
			if ( empty( $user_data ) )
				$errors->add( 'invalid_email', __( '<strong>Lỗi</strong>: Không có tài khoản nào được đăng ký với địa chỉ mail này.', 'theme-my-login' ) );
		} else {
		...
		if ( ! $user_data ) {
			$errors->add( 'invalidcombo', __( '<strong>Lỗi</strong>: Tên người dùng hoặc e-mail không hợp lệ.', 'theme-my-login' ) );
			return $errors;
		}
