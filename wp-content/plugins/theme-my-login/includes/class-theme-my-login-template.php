...
class Theme_My_Login_Template extends Theme_My_Login_Abstract {
	...
	public static function get_action_template_message( $action = '' ) {
		switch ( $action ) {
			case 'register':
				$message = __( 'Đăng ký thành viên cho vaycaptoc.com', 'theme-my-login' );
				break;
			case 'lostpassword':
				$message = __( 'Vui lòng nhập tên người dùng hoặc địa chỉ email của bạn. Bạn sẽ nhận được một liên kết để tạo một mật khẩu mới qua email.', 'theme-my-login' );
				break;
			case 'resetpass':
				$message = __( 'Nhập lại mật khẩu mới của bạn bên dưới.', 'theme-my-login' );
				break;
			default:
				$message = '';
		}
